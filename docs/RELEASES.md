...
3. Require contract/test success, two clean-root builds, artifact comparison,
   locked manifest/Java/JNI compatibility checks, exact tunnel source and ELF
   checks, complete component/license/advisory evidence, and provenance.
...
4. The workflow creates one annotated tag and uploads the already verified
   bundle. The AAR and tunnel ZIP are jointly attested build subjects. License
   texts are carried as the deterministic top-level
...
copy or any expanded tunnel binary. Static acceptance is followed by a
canonical local API 37 emulator Xray campaign. Producer and consumer waivers
are removed only when their exact replacement evidence is accepted.
